...
namespace AI_3y3net
{
    ...
    public class AIWaypointNetworkEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            PatrolPoint network = (PatrolPoint)target;

            // Show the Display Mode Enumeration Selector
            network.DisplayMode = (PathDisplayModeAI)EditorGUILayout.EnumPopup("Display Mode", network.DisplayMode);

            // If we are in Paths display mode then display the integer sliders for the Start and End waypoint indices
            if (network.DisplayMode == PathDisplayModeAI.Paths)
            {
        ...
        void OnSceneGUI()
        {
            ...
            if (network.DisplayMode == PathDisplayModeAI.Connections)
            {
